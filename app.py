...
async def _login():
    ...
    print(f"[*] Autenticando com conta guest: {_ID}")
    ...
    try:
        ...
        async with aiohttp.ClientSession() as s:
            async with s.post('https://100067.connect.garena.com/oauth/guest/token/grant', 
                ...
                if r.status != 200:
                    print(f"[!] Erro OAuth: {r.status}")
                    raise Exception(f"OAuth {r.status}")
                d = await r.json()
                oid = d.get('open_id')
                atk = d.get('access_token')
                if not oid or not atk:
                    print("[!] Resposta OAuth inválida")
                    raise Exception("Invalid OAuth response")
                print(f"[+] Token obtido: {atk[:20]}...")
        ...
        async with aiohttp.ClientSession() as s:
            async with s.post('https://loginbp.ggpolarbear.com/MajorLogin', 
                data=ep, 
                headers=_Hr, 
                ssl=sx,
                timeout=aiohttp.ClientTimeout(total=10)
                ...
                if r.status != 200:
                    print(f"[!] Erro MajorLogin: {r.status}")
                    raise Exception(f"MajorLogin {r.status}")
                mr = await r.read()

        # Processar resposta
        mlr = _pbF(mr)
        tok = mlr.get(8, b'').decode()
        tgt = mlr.get(1, 0)
        k = mlr.get(22, b'')
        v = mlr.get(23, b'')
        ts = mlr.get(21, 0)
        url = mlr.get(10, b'').decode()

        if not tok or not tgt:
            print("[!] Resposta MajorLogin inválida")
            raise Exception("Invalid MajorLogin response")

        print(f"[+] MajorLogin bem sucedido")

        # PASSO 3: GetLoginData
        h2 = {**_Hr, 'Authorization': f'Bearer {tok}'}
        async with aiohttp.ClientSession() as s:
            async with s.post(f"{url}/GetLoginData", 
                data=ep, 
                headers=h2, 
                ssl=sx,
                timeout=aiohttp.ClientTimeout(total=10)
                ...
                if r.status != 200:
                    print(f"[!] Erro GetLoginData: {r.status}")
                    raise Exception(f"GetLoginData {r.status}")
                lr = await r.read()

        ld = _pbF(lr)
        if 14 not in ld:
            print("[!] Resposta GetLoginData inválida")
            raise Exception("Invalid GetLoginData response")

        ip, port = ld[14].decode().split(':')
        at = await _auth(int(tgt), tok, int(ts), k, v)

        print(f"[+] Account ID: {tgt}")
        print(f"[+] IP: {ip}:{port}")
        print(f"[+] Bot online!")
...
def route_s():
    ...
    try:
        sx = _sess()
        result = asyncio.run(_query(uid, sx))
...
def route_status():
    try:
        ...
        return jsonify({
            'status': 'online',
            'account_id': sx['account_id'],
            'expires_in': int(sx['exp'] - time.time()),
            'ip': f"{sx['ip']}:{sx['port']}"
        })
    except Exception as e:
...
if __name__ == '__main__':
    print("="*60)
    print("  FREE FIRE STATUS API - BY ITHALO")
    print("="*60)
    print(f"[*] Conta: {_ID}")
    print("[*] Iniciando API na porta 5000...")
    print("[*] Acesse: http://localhost:5000/s?uid=SEU_UID")
